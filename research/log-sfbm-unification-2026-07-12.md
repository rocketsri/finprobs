...
The findings above marked "testable" or "unverified" were followed up with direct computation: closed-form numerical checks of the derived claims, Monte Carlo simulation with known ground truth, and analysis of real market data pulled fresh for this investigation (Yahoo Finance daily OHLC, 15 years, S&P 500 index + 25 large-cap single names across sectors; no prior paper in this report's bibliography uses this exact dataset). Code and raw output are in the session scratchpad; results are summarized honestly below, including where a test complicated rather than confirmed a finding.

**1. λ² cancellation / T-lever (Direction 4) — confirmed exactly.** Evaluating the model's own covariance formula C(τ)=(ν²/2)[T^{2H}−τ^{2H}] directly: the ratio Cov(τ_short)/Cov(τ_VIX) was numerically invariant to λ² to 6 decimal places across a 200× range of λ² values (ratio = 2.058205 in every case, at H=0.10, T=0.5y), confirming λ² truly cancels. The same ratio was strictly monotone decreasing in T (13.28 near T≈τ_VIX down to 1.02 as T→10⁵), and converged to exactly 1 as T→∞ (the pure-fBM limit killing the lever) — both exactly as the derivation predicted. This is now a fully verified result, not just a derived one.

**2. Cross-sectional aggregation mechanism (Direction 2, Zarhali et al.) — confirmed via simulation.** Simulating log-vol<sub>i</sub>(t) = β·F(t) + X<sub>i</sub>(t) with a smooth common factor F (H<sub>F</sub>=0.15) and i.i.d. rough/cascade-like idiosyncratic components X<sub>i</sub> (H≈0.02), the cross-sectional average's estimated H rose smoothly from ≈0.041 (N<sub>s</sub>=1) toward the common factor's own H≈0.149 as N<sub>s</sub> grew, tracking closely with the paper's own β⁴N<sub>s</sub>≫10 threshold (H already at 0.095 by N<sub>s</sub>=10, where β⁴N<sub>s</sub>=1.3; converged to 0.148 by N<sub>s</sub>=1000, β⁴N<sub>s</sub>=130). The mechanism holds up under independent simulation, not just the paper's own derivation.

**3. Real single-stock vs. index roughness (Direction 2's empirical anchor) — confirmed by a simple estimator, complicated by a more careful one.** Using a Parkinson range-based daily log-volatility proxy (not true high-frequency realized variance — a real limitation) and a standard structure-function-slope Hurst estimator, the S&P 500 index came out clearly smoother (Ĥ=0.094) than every one of 25 individual large-cap stocks (range 0.047–0.076, mean 0.058) — a clean, independent replication of Direction 2's central claim on data none of the cited papers used. However, applying the more careful nested boundary model-selection test built for Direction 1 (AIC comparison of a power-law C(τ) fit against a log-covariance C(τ) fit) to the same data told a messier story: 21 of 26 series — including the index itself — statistically favored the H=0 (log-covariance/cascade) boundary once the extra-parameter penalty was accounted for, with fitted H collapsing to the numerical boundary in most cases. Only a handful of names (AMZN, CVX, XOM, GOOGL, META) showed a statistically preferred positive H. This discrepancy between two reasonable estimators applied to the *same* series is itself a real, honest finding — a direct empirical illustration of exactly the estimator-fragility concern Fukasawa et al. and LeBaron raised in the literature review: small-H roughness estimates are not robust to estimator choice, which is precisely why Direction 1's genuineness question matters and remains open.

**4. Monte Carlo validation of the repaired Direction-1 diagnostic — real power limitations found.** Simulating 150 independent paths under each of two known ground truths (self-affine power-law covariance at H=0.10; genuine log-covariance cascade) and applying the same nested AIC-based model-selection test: it correctly identified the genuine-cascade ground truth 78% of the time, but only correctly identified the H=0.10 power-law ground truth 50% of the time — statistically indistinguishable from chance at this sample size (N=1500) and lag range. This does not invalidate the repaired diagnostic's underlying logic (the nested reparametrization via the model's own H→0 limit is still mathematically sound), but it does mean this simplified implementation is not yet reliable enough to trust on real data without a larger sample, a wider range of lag scales, or the fully proper boundary-corrected likelihood-ratio test (Self–Liang mixture-χ²) rather than plain AIC comparison. This is an honest negative finding about the diagnostic's current practical readiness, not about the model.

**Not run, and why:** the branching-ratio comparison (Direction 2's economic hypothesis) needs tick-level order-flow data with participant identity, which is not publicly available; the SPX/VIX T-lever test (Direction 4) needs an options-pricing engine for log S-fBM plus real option-chain data, which was out of scope for this pass — both remain exactly as specified in the "what would most efficiently move this forward" section above.
