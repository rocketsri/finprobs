...
- **A true realized-variance re-estimation is not feasible with data available here.** The only intraday source accessible (Yahoo Finance) caps out at ~60 trading days of 5-minute bars — far short of the multi-year daily series the structure-function methodology needs to estimate roughness reliably.

## A new finding since publication: real-data shuffle-surrogate test complicates the picture further

Running Brandi & Di Matteo's actual shuffle-surrogate method (not the AIC covariance-fit proxy used in the original addendum) directly on the same 26 real series found curvature that survives shuffling — genuinely temporal — dominates overwhelmingly: 76.6%–99.7% temporal across single stocks (mean 93.8%), 85.7% for the index. This is a real result on real data, and it creates a three-way tension: it disagrees with Brandi & Di Matteo's own rBergomi finding (mostly *distributional* at comparably low H), and it disagrees with Zhou (2009)/Jiang-Xie-Zhou-Sornette's finding that observed market multifractality is mainly distributional. Caveats: this is a single-stage shuffle test (not their full two-stage fBM-surrogate-then-shuffle procedure), uses a simplified quadratic-curvature measure, and a few series have near-zero raw curvature making the percentage split numerically unstable for those names. Treat this as a new data point complicating the debate, not resolving it — the natural next step is the full two-stage replication.
